...
const ApeyBoosterWhitelistChecker = artifacts.require("./ApeyBoosterWhitelistChecker.sol");
...
const ApeyBooster = artifacts.require(".test/ApeyBooster.sol");
...
contract(
  ...
  ([owner, admin, treasury, buyer1, buyer2, buyer3, seller1, seller2, seller3, creator1]) => {
    ...
    before(async () => {
      ...
      // Deploy ApeyBooster (modified implementation in Solidity 0.8)
      ApeyBooster = await ApeyBooster.new({ from: owner });

      // Deploy ApeyBoosterChecker
      ApeyBoosterChecker = await ApeyBoosterWhitelistChecker.new(ApeyBooster.address, { from: owner });
    ...
    describe("COLLECTIBLE MARKET #3 - TOKEN RESTRICTIONS/ApeyBooster", async () => {
      it("Add collection with restrictions", async () => {
        const result = await collectibleMarket.addCollection(
          ApeyBooster.address,
          constants.ZERO_ADDRESS,
          ApeyBoosterChecker.address,
          "100", // 1%
          "0",
          { from: admin }
        );
        ...
        expectEvent(result, "CollectionNew", {
          collection: ApeyBooster.address,
          creator: constants.ZERO_ADDRESS,
          whitelistChecker: ApeyBoosterChecker.address,
          tradingFee: "100",
          creatorFee: "0",
        });

        assert.equal(await ApeyBoosterChecker.canList("1"), true);
        assert.equal(await ApeyBoosterChecker.canList("2"), true);
        assert.equal(await ApeyBoosterChecker.canList("3"), true);
        assert.equal(await ApeyBoosterChecker.canList("4"), true);
        assert.equal(await ApeyBoosterChecker.canList("211"), true);

        const tokenListingStatuses = await collectibleMarket.canTokensBeListed(ApeyBooster.address, [
          "0",
          "1",
          "2",
          "3",
      ...
      it("Owner mint bunnyId 1-5 for seller1 and owner adds restrictions for bunnyId 3/4", async () => {
        ...
        while (i < 5) {
          await ApeyBooster.mint(seller1, "ipfs://" + i.toString(), i, { from: owner });
          i++;
        }

        const result = await ApeyBoosterChecker.addRestrictionForBunnies([new BN("3"), new BN("4")]);
        expectEvent(result, "NewRestriction");

        assert.equal(await ApeyBoosterChecker.isBunnyIdRestricted("3"), true);
        assert.equal(await ApeyBoosterChecker.isBunnyIdRestricted("4"), true);

        // For convenience, tokenId = 0 --> bunnyId = 0, tokenId = 1 --> bunnyId = 1
        assert.equal(await ApeyBoosterChecker.canList("3"), false);
        assert.equal(await ApeyBoosterChecker.canList("4"), false);

        const tokenListingStatuses = await collectibleMarket.canTokensBeListed(ApeyBooster.address, ["3", "4"]);
        const boolArray = Array.from({ length: 2 }, (i) => (i = false));
        assert.sameOrderedMembers(tokenListingStatuses, boolArray);
      });

      it("Seller 1 can sell tokenIds 0-2 (bunnyIds 0-2)", async () => {
        await ApeyBooster.setApprovalForAll(collectibleMarket.address, true, { from: seller1 });

        let result = await collectibleMarket.createAskOrder(ApeyBooster.address, "0", parseEther("1"), {
          from: seller1,
        });
        ...
        result = await collectibleMarket.createAskOrder(ApeyBooster.address, "1", parseEther("1"), {
          from: seller1,
        });
        ...
        result = await collectibleMarket.createAskOrder(ApeyBooster.address, "2", parseEther("1"), {
          from: seller1,
        });
      ...
      it("Seller 1 cannot sell tokenIds 3-4 (bunnyIds 3-4)", async () => {
        await expectRevert(
          collectibleMarket.createAskOrder(ApeyBooster.address, "3", parseEther("1"), {
            from: seller1,
          }),
        ...
        await expectRevert(
          collectibleMarket.createAskOrder(ApeyBooster.address, "4", parseEther("1"), {
            from: seller1,
          }),
          "Order: tokenId not eligible"
        );
      ...
      it("Owner removes restrictions for bunnyId=4", async () => {
        let result = await ApeyBoosterChecker.removeRestrictionForBunnies([new BN("4")]);
        expectEvent(result, "RemoveRestriction");

        assert.equal(await ApeyBoosterChecker.isBunnyIdRestricted("3"), true);
        assert.equal(await ApeyBoosterChecker.isBunnyIdRestricted("4"), false);
        assert.equal(await ApeyBoosterChecker.canList("3"), false);
        assert.equal(await ApeyBoosterChecker.canList("4"), true);
        ...
        result = await collectibleMarket.createAskOrder(ApeyBooster.address, "4", parseEther("1"), {
          from: seller1,
        });
      ...
      it("Revert statements work as expected", async () => {
        await expectRevert(
          ApeyBoosterChecker.removeRestrictionForBunnies([new BN("3"), new BN("4")], { from: owner }),
          "Operations: Not restricted"
        );

        await expectRevert(
          ApeyBoosterChecker.addRestrictionForBunnies([new BN("3"), new BN("4")], { from: owner }),
